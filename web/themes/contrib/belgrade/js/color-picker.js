/**
 * @file
 * Provides UI/UX progressive enhancements on Belgrade's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.belgrade.colorSchemes;
  ...
  function synchronizeInputs(changedInput, inputToSync) {
    ...
    changedInput.setAttribute('data-belgrade-custom-color', changedInput.value);
    inputToSync.setAttribute('data-belgrade-custom-color', changedInput.value);
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-belgrade-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-belgrade-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes Belgrade theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-belgrade-custom-color',
      textInput.getAttribute('data-belgrade-custom-color'),
    );
  ...
  /**
   * Belgrade Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.belgradeColorPicker = {
    attach: () => {
      const colorSchemeSelect = once(
        'belgrade-color-picker',
        '[data-drupal-selector="edit-color-scheme"]',
      );
      ...
      const colorTextInputs = once(
        'belgrade-color-picker',
        '[data-drupal-selector="belgrade-color-picker"] input[type="text"]',
      );
